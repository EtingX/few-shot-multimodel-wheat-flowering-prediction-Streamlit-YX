...
LOGGING_NAME = "ultralytics_YX"
MACOS, LINUX, WINDOWS = (platform.system() == x for x in ["Darwin", "Linux", "Windows"])  # environment booleans
ARM64 = platform.machine() in ("arm64", "aarch64")  # ARM64 booleans
HELP_MSG = """
    Usage examples for running YOLOv8:

    1. Install the ultralytics_YX package:

        pip install ultralytics_YX
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date ultralytics_YX
            'default.yaml' file.\nPlease update your code with 'pip install -U ultralytics_YX' and if necessary replace
...
class ThreadingLocked:
    ...
    Example:
        ```python
        from ultralytics_YX.utils import ThreadingLocked
...
class SettingsManager(dict):
    ...
    def __init__(self, file=SETTINGS_YAML, version="0.0.4"):
        ...
        from ultralytics_YX.utils.checks import check_version
        from ultralytics_YX.utils.torch_utils import torch_distributed_zero_first
        ...
        with torch_distributed_zero_first(RANK):
            ...
            if not (correct_keys and correct_types and correct_version):
                LOGGER.warning(
                    ...
                    f"with your settings or a recent ultralytics_YX package update. {help_msg}"
                )
                self.reset()
...
def deprecation_warn(arg, new_arg, version=None):
    ...
    LOGGER.warning(
        f"WARNING ⚠️ '{arg}' is deprecated and will be removed in 'ultralytics_YX {version}' in the future. "
